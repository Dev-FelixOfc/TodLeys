...
export const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        // Guard: si el WebSocket ya cerró, no tiene sentido hacer la query USync —
        // lanzaría 428 Connection Closed de todos modos. Retornamos lo que ya hay en caché.
        if (!sock.ws?.isOpen) {
            logger.warn({ toFetch }, 'getUSyncDevices: ws not open, skipping USync query — returning cached results only');
            return deviceResults;
        }
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                // Guard: verificar que la conexión sigue activa antes de intentar
                // obtener devices vía USync — evita el error 428 en grupos/estados
                // cuando el ws cae justo mientras se procesa un mensaje entrante.
                if (!sock.ws?.isOpen) {
