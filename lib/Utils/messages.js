...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        // generate JPEG thumbnail and upload separately as image type
        let thumbBuffer;
        try {
            const { getImageProcessingLibrary } = await import('./messages-media.js');
            const lib = await getImageProcessingLibrary();
            if ('sharp' in lib && lib.sharp?.default) {
                thumbBuffer = await lib.sharp.default(coverBuffer)
                    .resize(96, 96, { fit: 'cover', position: 'center' })
                    .jpeg({ quality: 80 })
                    .toBuffer();
            } else {
                thumbBuffer = coverBuffer;
            }
        } catch (_) {
            thumbBuffer = coverBuffer;
        }
        const thumbUpload = await encryptedStream(thumbBuffer, 'image', {
            logger: options.logger,
            opts: options.options
        });
