...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        const lib = await getImageProcessingLibrary().catch(() => null);
        ...
        const stickerPromises = stickers.map(async (s, i) => {
            ...
            if (isWebPBuffer(buffer)) {
                ...
                const jimpImage = await (lib.jimp.Jimp || lib.jimp.default).read(buffer);
                webpBuffer = await jimpImage.getBuffer('image/webp');
            } else {
                webpBuffer = buffer;
            }
