...
export const generateProfilePicture = async (mediaUpload) => {
    ...
    if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
        ...
        const cropped = jimp.crop({ x: 0, y: 0, w: jimp.width, h: jimp.height });
        img = cropped.resize({ w: 720, h: 720, mode: lib.jimp.ResizeStrategy.BILINEAR }).getBuffer('image/jpeg', { quality: 100 });
    }
    else {
        throw new Boom('No image processing library available');
    }
